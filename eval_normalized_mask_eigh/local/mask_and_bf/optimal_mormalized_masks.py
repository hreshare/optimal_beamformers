...
# num-of-masks, mode_idx, do_minimize
MODE_BOTH = 0
MODE_LEFT = 1
MODE_RIGHT = 2
MODE_IDEAL_MAX = 3
...
MODE_INFO = {
    'both_max': (2, MODE_BOTH, False),
    'both_min': (2, MODE_BOTH, True),
    'left_max': (1, MODE_LEFT, False),
    'left_min': (1, MODE_LEFT, True),
    'right_max': (1, MODE_RIGHT, False),
    'right_min': (1, MODE_RIGHT, True),
    'ideal_max': (0, MODE_IDEAL_MAX, False),
    'ideal_min': (0, MODE_IDEAL_MIN, True),
}


class MaskBfEstimator(nn.Module):
    def __init__(self, bins, frames, channels, mode):
        super().__init__()
        (masks_num, mode_idx, minimize) = MODE_INFO[mode]
    ...
    def get_normalized_masks(self):
        ...
        if masks.shape[1] > 0:
            ...
            masks = masks / var.sqrt()
        return masks

    def calc_scaled_bf(self, x, s, target_ch):
        bf_out = self._mask_based_bf(x, s, target_ch)
        return bf_out

    def calc_loss(self, X, S, target_ch):
        """
        X.shape = S.shape = (frames, mics, bins)
        """
        bf_scaled = self._mask_based_bf(X, S, target_ch)   # shape: (frames, 1, bins)
        S_target = S[:,target_ch:target_ch+1]   # shape: (frames, 1, bins)
    ...
    def _mask_based_bf(self, x, s, target_ch):
        """
        ...
        if self.mode_idx == MODE_IDEAL_MIN: # 'ideal_min':
            wcov1 = torch.matmul(x, htp_x) / frames
            wcov2 = torch.matmul(x, self._htp(s)) / frames
        elif self.mode_idx == MODE_IDEAL_MAX: # 'ideal_max':
            wcov1 = torch.matmul(x, self._htp(s)) / frames
            wcov2 = torch.matmul(x, htp_x) / frames
        elif self.mode_idx == MODE_BOTH: # startswith('both_'):
            wcov1 = self._calc_wcov(x, masks[:,0,:])
            wcov2 = self._calc_wcov(x, masks[:,1,:])
        elif self.mode_idx == MODE_LEFT: # startswith('left_'):
            wcov1 = self._calc_wcov(x, masks[:,0,:])
            wcov2 = torch.matmul(x, htp_x) / frames
        else: # startswith('right_'):
            wcov1 = torch.matmul(x, htp_x) / frames
            wcov2 = self._calc_wcov(x, masks[:,0,:])

        w = self._gevh(wcov1, wcov2, self.minimize)
        y = torch.matmul(self._htp(w), x)
        #y = self._rescale(y, x[:,target_ch:target_ch+1])
        y = self._rescale(y, s[:,target_ch:target_ch+1])
    ...
    # solve Ax = dBx  by using eigh()
    def _gevh(self, A, B, minimize):
        #D, Q = torch.linalg.eigh(B)
        #D = D.unsqueeze(-1)
        ##htp_P = Q / D.sqrt()
        #htp_P = torch.linalg.pinv(Q * D.sqrt())
        #P = self._htp(htp_P)   # P = diag(D)^(-1/2) Q^H
        L = torch.linalg.cholesky(B)
        P = torch.linalg.pinv(L)
        htp_P = self._htp(P)
        PAP = torch.matmul(torch.matmul(P, A), htp_P)
        _, V = torch.linalg.eigh(PAP)
        if minimize:
            v = V[...,:1]
        else:
            v = V[...,-1:]
        return torch.matmul(htp_P, v)
...
for cur_line in tqdm(flist[:last_idx]):
    ...
    model = MaskBfEstimator(bins=bins, frames=frames, channels=channels, mode=args.mode)
    if args.gpu >= 0:
        model.cuda(args.gpu)
        X = X.cuda(args.gpu)
        S = S.cuda(args.gpu)
    ...
    if args.mode.startswith('ideal_'):
        with Timer() as t:
            loss = model.calc_loss(X, S, target_ch=target_ch)
        t_fw += t.msecs
    else:
        model.train()
    ...
    with Timer() as t:
        with torch.no_grad():  ## disable autograd
            Y = model.calc_scaled_bf(X, S, target_ch)
    t_net += t.msecs

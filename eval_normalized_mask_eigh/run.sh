...
stage=1
#stage=3
enhancement_base=normalized_masks_eigh #### or your method
...
# Gain of background noise
bg_gain=1.0

#do_plot=True
do_plot=False

# <|m2|^2 xx^H>w = λ_max <|m1|^2 xx^H> w		both_max
# <|m1|^2 xx^H>w = λ_min <|m2|^2 xx^H> w		both_min
# <xx^H>w       = λ_max <|m1|^2 xx^H> w		right_max
# <|m1|^2 xx^H>w = λ_min <xx^H> w			left_min
# <xx^H>w       = λ_min <|m2|^2 xx^H> w		right_min
# <|m2|^2 xx^H>w = λ_max <xx^H> w			left_max
# <xx^H>w = λ_min <ss^H> w			ideal_min
# <ss^H>w = λ_max <xx^H> w			ideal_max
...
chime4_data=/data/CHiME4
chime3_data=/data/CHiME4
...
enhancement=${enhancement_base}_bg-${bg_gain}_mode-${mode}
enhancement_data=`pwd`/enhan/${enhancement}
if [ $stage -le 1 ]; then
    local/run_train_and_infer.sh \
      --cmd "$train_cmd" \
      --nj 20 \
      $chime4_data $chime3_data $enhancement_data \
      $bg_gain ${mode} $do_plot
fi
...
if [ $stage -le 2 ]; then
  ...
  #fi
  chime4_rir_data=local/mask_and_bf/data/audio/16kHz/isolated_ext

...
avg_mos=0
  while read filename; do
    abs_fn=`realpath $filename`
    #tmp_fn=`mktemp --dry-run --tmpdir=$tmpdir --suffix=".$$.wav"`
    tmp_fn=`mktemp --dry-run --suffix=".$$.wav"`
    ln -s $abs_fn $tmp_fn
    ...
    temp=`$modeldir/local/PESQ +16000 ../../$chime_rir_directory/"$set"_"$noise"_simu/"$speaker"_"$utt_id"_"$noise_cap".CH5.Clean.wav $tmp_fn`

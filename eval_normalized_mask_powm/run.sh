...
stage=1
#stage=3
enhancement_base=normalized_masks_powm #### or your method
...
# w <-- inv(<m1 xx^H>) <m2 xx^H> ek   both
# w <-- inv(<m1 xx^H>) <xx^H> ek      left
# w <-- inv(<xx^H>) <m2 xx^H> ek      right
# w <-- inv(<xx^H>) <ss^H> ek         ideal
...
enhancement=${enhancement_base}_bg-${bg_gain}_mode-${mode}_powm-${powm_itr}

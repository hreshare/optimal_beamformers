...
stage=1
#stage=3
enhancement_base=mask_transfer   #### or your method
...
enhancement=${enhancement_base}_bg-${bg_gain}_from-${mode_from}_to-${mode_to}_powm-${powm_itr}
...
if [ $stage -le 1 ]; then
    local/run_train_and_infer.sh \
      ...
      $bg_gain ${mode_from} ${mode_to} ${powm_itr} $do_plot
fi

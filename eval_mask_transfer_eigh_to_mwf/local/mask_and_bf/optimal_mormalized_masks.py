...
def mask_based_bf(X, S, mask, target_ch, mode_to, powm_itr):
    X = X.T
    S = S.T
    mask = mask.T
    Y, mask_after = _mask_based_bf(X, S, mask, target_ch, mode_to, powm_itr)
    return Y.T, mask_after.T

def _mask_based_bf(X, S, mask, target_ch, mode_to, powm_itr):
    if mode_to == MODE_TO_LEFT:
        mask = _flip_mask(mask)
    XX = np.matmul(X, htp(X))
    mXX = np.matmul(mask*X, htp(X))
    if mode_to == MODE_TO_LEFT:
        W = np.matmul(np.linalg.pinv(mXX), XX)
    else: # mode_to == MODE_TO_RIGHT
        W = np.matmul(np.linalg.pinv(XX), mXX)
    w = _power_method(W, target_ch, powm_itr)
    Y = np.matmul(htp(w), X)
...
def _flip_mask(x):
    return x.max(axis=-1, keepdims=True) - x
...
parser.add_argument('--mode_from',
                    help='Type of source mask. (left_max|mwf)')
parser.add_argument('--mode_to',
                    help='Type of destination mode. (left|right)')
parser.add_argument('--powm_itr', default=1, type=int,
                    help='Iteration count for the power method')
...
if args.mode_to not in (MODE_TO_LEFT, MODE_TO_RIGHT):
    raise ValueError('Unknoen destination mode:', args.mode_to)

if args.mode_from == MODE_FROM_LEFT_MAX:
...
elif args.mode_from == MODE_FROM_MWF:
    MASK_BASE_DIR = os.path.join(
        '..',
    ...
    raise ValueError('Unknoen destination mode:', args.mode_from)
...
for cur_line in tqdm(flist[:last_idx]):
    ...
    with Timer() as t:
        Y, mask_after = mask_based_bf(X, S, mask=mask, target_ch=target_ch,
                            mode_to=args.mode_to, powm_itr=args.powm_itr)
    t_bf += t.msecs

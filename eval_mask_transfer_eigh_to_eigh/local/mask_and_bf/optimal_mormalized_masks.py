...
MODE_LEFT_MAX = 'left_max'
MODE_LEFT_MIN = 'left_min'
MODE_RIGHT_MAX = 'right_max'
MODE_RIGHT_MIN = 'right_min'
...
def mask_based_bf(X, S, mask, target_ch, mode_from, mode_to):
    X = X.T
    S = S.T
    mask = mask.T
    Y, mask_after = _mask_based_bf(X, S, mask, target_ch, mode_from, mode_to)
    return Y.T, mask_after.T

def _mask_based_bf(X, S, mask, target_ch, mode_from, mode_to):
    if mode_from in (MODE_LEFT_MIN, MODE_RIGHT_MAX):
        mask = mask.max(axis=-1, keepdims=True) - mask
    if mode_to in (MODE_LEFT_MIN, MODE_RIGHT_MAX):
        mask = mask.max(axis=-1, keepdims=True) - mask

    XX = np.matmul(X, htp(X))
    mXX = np.matmul(mask*X, htp(X))
    if mode_to == MODE_LEFT_MAX:
        w = _gevh(mXX, XX, minimize=False)
    elif mode_to == MODE_LEFT_MIN:
        w = _gevh(mXX, XX, minimize=True)
    elif mode_to == MODE_RIGHT_MAX:
        w = _gevh(XX, mXX, minimize=False)
    elif mode_to == MODE_RIGHT_MIN:
        w = _gevh(XX, mXX, minimize=True)
    else:
        raise ValueError('Unknown mode_to', mode_to)
    Y = np.matmul(htp(w), X)
...
def _flip_mask(mask):
    return mask.max(axis=-1, keepdims=True) - mask
...
parser.add_argument('--mode_from',
                    help='Mode of source mask. (left_max|left_min|right_max|right_min)')
parser.add_argument('--mode_to',
                    help='Mode of destination BF. (left_max|left_min|right_max|right_min)')
...
if args.mode_from not in (MODE_LEFT_MAX,  MODE_LEFT_MIN, MODE_RIGHT_MAX, MODE_RIGHT_MIN):
    raise ValueError('Unknoen mode_from:', args.mode_from)
if args.mode_to not in (MODE_LEFT_MAX,  MODE_LEFT_MIN, MODE_RIGHT_MAX, MODE_RIGHT_MIN):
    raise ValueError('Unknoen mode_to:', args.mode_to)

MASK_BASE_DIR = os.path.join(
    '..',
    'eval_normalized_masks_eigh',
    'enhan',
    'normalized_masks_eigh_bg-{}_mode-{}'.format(args.bg_gain, args.mode_from),
)
...
for cur_line in tqdm(flist[:last_idx]):
    ...
    with Timer() as t:
        Y, mask_after = mask_based_bf(X, S, mask=mask, target_ch=target_ch,
                                mode_from=args.mode_from, mode_to=args.mode_to)
    t_bf += t.msecs
